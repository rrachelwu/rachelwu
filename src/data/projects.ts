...
export const projects: Project[] = [
  ...
  },
  {
    ...
    summary: "面向安全研究员的代码漏洞检测大模型——用户提交代码，系统在30分钟内完成10万行代码的漏洞扫描并输出检测报告。如何把 LLM 生成的非结构化分析结果，转化为工程师可以高效消费的结构化界面，是这个项目的核心设计命题。",
    summaryEn: "A code vulnerability detection LLM for security researchers — users submit code, the system scans 100k lines within 30 minutes and outputs detection reports. How to transform unstructured LLM-generated analysis into a structured interface that engineers can consume efficiently was the core design question of this project.",
    role: "UI/UX设计师",
    ...
    links: {},
    background: "武汉金银湖实验室是依托华中科技大学、武汉大学等顶尖高校，在国家网络安全人才与创新基地建立的新型研发机构，由武汉市人民政府批复设立。核心产品是面向网络安全领域的代码漏洞检测大模型——通过学习30多万个「漏洞切片」，实现从百万行代码中自动捕捉漏洞，10万行代码扫描不超过30分钟。我作为外包设计师独立负责该平台全部UI设计，用户是安全研究员和工程师。独立完成高保真设计稿后，主动推动线下与客户现场确认，整体方案一稿通过，后续只做局部微调。因签署保密协议，完整页面不对外展示。这个产品的核心设计命题是：如何把 LLM 生成的非结构化输出（漏洞分析、风险判断、代码片段），转化为安全工程师可以高效消费的结构化 Web 界面——让专业用户在 GUI 环境里获得和命令行工具同等的掌控感与信息透明度。",
    backgroundEn: "Wuhan Jinyinhu Laboratory is a new R&D institution established at the National Cybersecurity Talent and Innovation Base, approved by the Wuhan Municipal Government and built with HUST and Wuhan University. Its core product is a code vulnerability detection LLM — trained on 300,000+ vulnerability slices to automatically detect vulnerabilities in millions of lines of code, scanning 100k lines in under 30 minutes. As a freelance designer, I independently handled all UI design for the platform. After independently completing high-fidelity designs, I proactively initiated an in-person client review session — the overall concept passed in one round with only minor local revisions afterward. Full pages are not publicly displayed due to NDA. The core design question of this product: how to transform unstructured LLM output (vulnerability analysis, risk assessment, code snippets) into a structured web interface that security engineers can consume efficiently — giving professional users the same sense of control and information transparency in a GUI environment as they have with command-line tools.",
    decisions: [
      {
        problem: "漏洞检测结果是 LLM 生成的非结构化内容，包含分析文字、风险判断、代码片段混排，工程师难以快速定位关键信息",
        judgment: "安全工程师看漏洞报告的核心诉求是：快速定位漏洞位置、理解风险等级、获取代码级别的修复依据。LLM 的输出没有天然的视觉层级，如果直接呈现会让工程师陷入逐行阅读，效率极低",
        solution: "设计三层信息架构：漏洞列表（风险等级标签+漏洞名称）→ 漏洞详情（上下文分析文字）→ 代码块（具体漏洞片段）。三层之间有明确的视觉分界，工程师可以在列表层快速扫描，在详情层理解上下文，在代码层获取精确位置，不需要从头逐字阅读",
      },
      {
        problem: "代码块是漏洞报告的核心内容，但普通文字界面无法区分代码和分析文字",
        judgment: "工程师看漏洞报告时，代码片段是最关键的证据——它指向具体的漏洞行。如果代码和普通文字混排没有视觉区分，工程师需要额外的认知成本来识别哪些是代码、哪些是分析",
        solution: "设计专属代码块组件：深色背景与白色正文区形成强对比，等宽字体保证代码可读性，保留原始代码格式不做过度美化。让工程师在视觉上瞬间识别代码区域，和阅读命令行输出一样直觉化",
      },
      {
        problem: "LLM 分析代码需要时间，用户提交任务后进入等待状态，这个过程容易让用户感到焦虑或不信任",
        judgment: "安全工程师提交的是真实业务代码，等待过程中他们需要知道：任务是否在正常处理、预计需要多久、有没有出错。如果等待界面是一个空白加载圈，专业用户会直接怀疑系统是否在正常工作",
        solution: "设计任务状态可见机制：提交后显示任务进入队列的确认状态，处理中显示当前阶段进度，完成后以醒目的方式通知用户。让用户在等待过程中始终知道系统在做什么，把 LLM 的处理过程从黑箱变成可追踪的任务流",
      },
      {
        problem: "这是一个 LLM 驱动的代码审计平台，用户习惯了命令行工具的「可见、可控、可预期」，GUI 界面容易让他们感到失控",
        judgment: "CLI 工具的核心体验是每个操作都有即时反馈，用户清楚知道系统在做什么。把这套能力做成 Web 平台时，最大的风险是把 LLM 的处理过程变成黑箱——用户提交代码后不知道发生了什么",
        solution: "在界面设计上保留「CLI 的信息透明度」：检测任务状态实时可见，LLM 分析结果完整展示不做截断，历史记录支持回溯查看。让工程师在 Web 界面里获得和命令行工具一样的掌控感，而不是面对一个「提交后等待魔法发生」的黑箱产品",
      },
    ],
    decisionsEn: [
      {
        problem: "Vulnerability detection results are unstructured LLM-generated content mixing analytical text, risk assessments, and code snippets — engineers struggle to quickly locate key information",
        judgment: "Security engineers reading vulnerability reports need to: quickly locate vulnerability positions, understand risk levels, and get code-level fix references. LLM output has no natural visual hierarchy — presenting it directly forces engineers into line-by-line reading, extremely inefficient",
        solution: "Designed three-level information architecture: vulnerability list (risk level tags + vulnerability names) → vulnerability details (contextual analysis text) → code blocks (specific vulnerability snippets). Clear visual boundaries between three levels allow engineers to scan at list level, understand context at detail level, and get precise locations at code level — no need to read from the start",
      },
      {
        problem: "Code blocks are the core content of vulnerability reports, but plain text interfaces cannot distinguish code from analytical text",
        judgment: "When engineers read vulnerability reports, code snippets are the most critical evidence — they point to specific vulnerable lines. Without visual distinction between code and regular text, engineers need extra cognitive effort to identify what is code and what is analysis",
        solution: "Designed a dedicated code block component: dark background creates strong contrast with white body text, monospace font ensures code readability, original code formatting preserved without over-beautification. Engineers instantly recognize code areas visually — as intuitive as reading command-line output",
      },
      {
        problem: "LLM analysis takes time — users enter a waiting state after submitting tasks, which easily causes anxiety or distrust",
        judgment: "Security engineers submit real business code — during the wait they need to know: is the task processing normally, how long will it take, has anything gone wrong. If the waiting interface is just a blank loading spinner, professional users will immediately doubt whether the system is working correctly",
        solution: "Designed a task status visibility mechanism: submission confirmation showing task entered queue, in-progress display showing current processing stage, prominent notification when complete. Users always know what the system is doing during the wait — transforming LLM processing from a black box into a trackable task flow",
      },
      {
        problem: "This is an LLM-driven code audit platform, but users accustomed to CLI tools expect visible, controllable, predictable experiences — GUI interfaces easily make them feel out of control",
        judgment: "The core CLI experience is immediate feedback for every action — users always know what the system is doing. The biggest risk when building this as a web platform is turning LLM processing into a black box — users submit code and do not know what happens next",
        solution: "Preserved CLI transparency in the interface design: detection task status visible in real-time, LLM analysis results displayed completely without truncation, history records support retrospective viewing. Engineers get the same sense of control in the web interface as command-line tools — not a black box where magic happens after submission",
      },
    ],
    deliverables: [],
    deliverablesEn: [],
    results: [
      "独立完成 13 个页面全部 UI 设计，覆盖注册登录、任务提交、漏洞列表、漏洞详情、代码块输出、历史记录完整闭环",
      "基于对网络安全工具产品形态的理解，独立完成高保真设计稿后带到线下与客户现场确认，整体方案一稿通过，后续只做局部微调",
      "三层漏洞报告信息架构和代码块组件获客户认可，作为产品核心交互模式落地",
      "任务状态可见机制设计完整交付，把 LLM 处理过程从黑箱变成可追踪的任务流",
      "产品内部顺利上线，服务武汉金银湖实验室安全大模型研发团队",
    ],
    resultsEn: [
      "Independently completed all 13 pages of UI design, covering the complete loop of registration, task submission, vulnerability list, vulnerability details, code block output, and history records",
      "Based on understanding of cybersecurity tool product patterns, independently completed high-fidelity designs before in-person client review — overall concept passed in one round with only minor local revisions",
      "Three-level vulnerability report IA and code block component approved by client, implemented as core interaction pattern",
      "Task status visibility mechanism fully delivered, transforming LLM processing from a black box into a trackable task flow",
      "Product successfully launched internally, serving Jinyinhu Lab's security LLM research team",
    ],
    reflection: "这个项目让我第一次系统思考「LLM 的输出如何被界面结构化」这个问题。大模型生成的内容和传统软件的结构化数据完全不同——它是非确定性的、长度不固定的、混合多种内容类型的。如何为这种输出设计一套稳定的呈现规则，是 AI 产品界面设计的核心挑战之一。这个项目给了我一次真实的实践机会，也让我意识到：AI 产品的设计师需要比传统产品设计师更深地理解模型的输出边界，才能做出真正有用的界面。",
    reflectionEn: "This project was my first systematic thinking about how LLM output can be structured through interface design. Content generated by large models is fundamentally different from traditional software's structured data — it is non-deterministic, variable in length, and mixes multiple content types. Designing a stable presentation framework for this kind of output is one of the core challenges in AI product interface design. This project gave me a real practice opportunity and made me realize: AI product designers need to understand model output boundaries more deeply than traditional product designers to create truly useful interfaces.",
  },
  {
    id: "1",
    slug: "ssrouter",
    ...
    deliverables: [
      {
        ...
        image: "/images/ssrouter/brand-identity-v2.png",
        caption: "品牌基础",
      },
      {
