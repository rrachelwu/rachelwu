...
export const projects: Project[] = [
  ...
  },
  {
    ...
    links: {},
    background: "武汉金银湖实验室是依托华中科技大学、武汉大学等顶尖高校，在国家网络安全人才与创新基地建立的新型研发机构，由武汉市人民政府批复设立。核心产品是面向网络安全领域的代码漏洞检测大模型——通过学习30多万个「漏洞切片」，实现从百万行代码中自动捕捉漏洞，10万行代码扫描不超过30分钟。我作为外包设计师独立负责该平台全部UI设计，用户是安全研究员和工程师。项目中途主动推动了一次线下需求对齐，明确核心使用场景后进入高保真设计阶段，整体方案一稿通过。因签署保密协议，完整页面不对外展示。这个产品的核心设计命题是：如何把 LLM 生成的非结构化输出（漏洞分析、风险判断、代码片段），转化为安全工程师可以高效消费的结构化 Web 界面——让专业用户在 GUI 环境里获得和命令行工具同等的掌控感与信息透明度。",
    backgroundEn: "Wuhan Jinyinhu Laboratory is a new R&D institution established at the National Cybersecurity Talent and Innovation Base, approved by the Wuhan Municipal Government and built with HUST and Wuhan University. Its core product is a code vulnerability detection LLM — trained on 300,000+ vulnerability slices to automatically detect vulnerabilities in millions of lines of code, scanning 100k lines in under 30 minutes. As a freelance designer, I independently handled all UI design for the platform. Mid-project, I proactively initiated an in-person requirement alignment session to clarify core use cases before entering high-fidelity design — the overall concept passed in one round. Full pages are not publicly displayed due to NDA. The core design question of this product: how to transform unstructured LLM output (vulnerability analysis, risk assessment, code snippets) into a structured web interface that security engineers can consume efficiently — giving professional users the same sense of control and information transparency in a GUI environment as they have with command-line tools.",
    decisions: [
      {
